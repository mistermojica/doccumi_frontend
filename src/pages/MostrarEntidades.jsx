...
const MostrarVehículos = () => {
  ...
  const GetAllData = (ctxProId) => {
    const url = Config.gatDomainName()
      .concat('/entidades/listapordueno/')
      .concat(ctxProId);
    axios
      .get(url)
      .then((response) => {
  ...
  const submitData = () => {
    const url = Config.gatDomainName().concat('/entidades/update');
    const body = {
      _id,
      vehNoRegistroPlaca,
      vehChasis,
  ...
  const handleDelete = () => {
    const url = Config.gatDomainName().concat(`/entidades/delete/${_id}`);
    axios
      .get(url)
      .then((response) => {
        const result = response.data;
