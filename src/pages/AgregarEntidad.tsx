...
import EntidadesFormBody from '@app/components/forms/EntidadesFormBody';
...
const AgregarEntidad = () => {
  ...
  const [vehStatusEntidad, setStatusEntidad] = useState('');
  const [vehTipoEmision, setTipoEmision] = useState('');
  const [vehTipoEntidad, setTipoEntidad] = useState('');
  ...
  const setFieldsValues = (name: string, value: any) => {
    ...
    switch (name) {
      ...
      case 'vehChasis':
        setChasis(value);
        break;
      case 'vehStatusEntidad':
        setStatusEntidad(value);
        break;
      case 'vehTipoEmision':
  ...
  const handleAddNew = () => {
    const url = Config.gatDomainName().concat('/entidades/create');
    const body = {
      vehNoRegistroPlaca,
      vehChasis,
      vehStatusEntidad,
    ...
    console.log('AgregarEntidad() || body:', body);
